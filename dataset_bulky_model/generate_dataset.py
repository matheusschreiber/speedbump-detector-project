...
def generate_sample(targets_path, img_name, templates, probabilities_vector, positions_list, images_out_path,
    ...
    pascal_voc(out_path, f"{os.path.splitext(img_name)[0]}.jpg", img_out_path, bboxes, nb_imgs_generated)
...
def pascal_voc(out_path, img_name, img_out_path, bboxes, nb_imgs_generated):
    ...
    img_in_path = os.path.join('bgs', img_name)
    image = cv2.imread(img_in_path)
    img_name = img_in_path.split('/')[-1]
    ...
    out_path = img_out_path.split('/')[2].split('.')[0]
    anno_output_path = os.path.join(annotations_path, f"{out_path}.xml")
...
def parse_args():
    ...
    parser.add_argument('--bgs-path',       dest='targets_path',    type=str, required=True, help='Path to the directory containing background images to be used.')
    parser.add_argument('--templates-path', dest='templates_path',  type=str, required=True, help='Path to the directory containing template images to be used.')
    parser.add_argument('--out-path',       dest='out_path',        type=str, required=True, help='Path to the directory to save the images generated to.')
    parser.add_argument('--total-images',   dest='total_images',    type=int, required=True, help='Number of images to be generated.')
    parser.add_argument('--garbage-index',  dest='garbage_index',   type=int, default=0, help='Index that indicates the start of garbage templates.')
    parser.add_argument('--data',           dest='random_data',     type=str, default=None, help='Path to data directory (to reproduce a data set, not working perfectly at the moment, will be at future version)')
    parser.add_argument('--max_process',    dest='max_process',     type=int, default=1, help='Maximum number of parallel processes')
...
if __name__ == '__main__':
    ...
    for idx, img_name in enumerate(img_names):
        p = mp.Process(target=generate_sample, args=((
            ...
            binary_annotation_lines_queue, multiclass_annotation_lines_queue, data_out_path, load_path, args.out_path)))
        p.daemon = True
        p.start()
        processes.append(p)
