...
def geometric_transform(template, template_mask, x, target_size=(1500, 1500), scale=None, prelodaded_data=None):
    ...
    return template, template_mask, scale_factor, data, M
...
def process_img(target, template, template_mask, probabilities_vector, positions_list, multiply_value, position=None,
    ...
    template, template_mask, scale, geometric_transform_data, M = geometric_transform(template, template_mask, x,
                                                                                   scale=scale,
    ...
    points = [  [  x0,             height/2 + y0 ],
                [  width/2 + x0,   y0            ],
                [  x1,             height/2 + y0 ],
                [  width/2 + x0,   y1            ]]

    center = [
        (max(points[0][0],points[1][0],points[2][0],points[3][0]) + min(points[0][0],points[1][0],points[2][0],points[3][0])) / 2.0,
        (max(points[0][1],points[1][1],points[2][1],points[3][1]) + min(points[0][1],points[1][1],points[2][1],points[3][1])) / 2.0
    ]

    M = np.array([[M.item(0), -M.item(1)],[-M.item(3),M.item(4)]])

    for p in points:
        p[0] -= center[0]
        p[1] -= center[1]

        array = np.array([p[0], p[1]])
        array = np.dot(array, M)
...
def generate_sample(targets_path, img_name, templates, probabilities_vector, positions_list, images_out_path,
    ...
    for bbox in bboxes:
        ...
        aux = []
        for p in points:
            aux.append({"x":p[0], "y":p[1]})

        polygons.append({"hole":"false", "points":aux})

        binary_line = f"{img_out_path},{points[0][0]},{points[0][1]},{points[1][0]},{points[1][1]},{points[2][0]},{points[2][1]},{points[3][0]},{points[3][1]},sb_sign"
        multiclass_line = f"{img_out_path},{points[0][0]},{points[0][1]},{points[1][0]},{points[1][1]},{points[2][0]},{points[2][1]},{points[3][0]},{points[3][1]},{bbox['category']}"
        
        binary_annotation_lines.append(binary_line)
        multiclass_annotation_lines.append(multiclass_line)
    
    label = [{
        "name":f"{nb_imgs_generated:05d}_{img_name}",
        "polygonGroups":[{
            "polygons": polygons,
            "occluded": "false",
            "label": "sb_sign",
